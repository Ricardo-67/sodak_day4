"""The whole project in one command, for the classroom.
    ...
    python -m scripts.demo --crash    # kill the run right after the complaint, resume, count
...
QUESTIONS = [
    ("22CS045", "I need to raise a plumbing complaint in my room and please text me."),
    ("22IT017", "Can I raise a fan complaint?"),
]
...
def main() -> None:
    ...
    os.environ["AGENT_DB"], os.environ["HOSTEL_DB"] = os.path.join(tmp, "agent.db"), os.path.join(tmp, "hostel.db")
        ...
        if a.crash:
            ...
            def once_then_die(key, tool_name, effect):
                ...
                if tool_name == "raise_complaint":
                    raise Crash()        # the complaint and its key are committed; nothing after is
                return result
