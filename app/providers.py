...
def demo_providers(slow: float = 0.0) -> dict:
    ...
    return {
        "supervisor": RoutedMock({
            "plumbing": [
                _call("ask_desk", request="Raise a plumbing complaint and text the student to confirm."),
                ModelTurn(text="(mock) Your plumbing complaint has been raised and a confirmation text is on its way."),
            ],
            "electrical": [
                _call("ask_information", question="Are there any electrical complaints in room A 101?"),
                _call("ask_desk", request="Raise an electrical complaint for the student."),
                ModelTurn(text="(mock) The student cannot raise another electrical complaint because there is already an open electrical issue in A 101."),
            ],
            "fan": [
                _call("ask_desk", request="Raise a fan complaint for the student."),
                ModelTurn(text="(mock) The desk cannot raise a complaint because the student has 3 warnings, which exceeds the limit of 2."),
            ],
        }, slow),
        "information": RoutedMock({
            "electrical": [_call("find_complaint", block="A", number="101", issue="electrical"),
                           ModelTurn(text="(mock) Room A 101 has an open electrical complaint (complaint_id 1).")],
        }, slow),
        "desk": RoutedMock({
            "plumbing": [
                _call("check_can_raise"),
                ModelTurn(text=None, tool_calls=[
                    ToolCall("raise_complaint", {"issue": "plumbing"}),
                    ToolCall("notify_student", {"message": "Your plumbing complaint has been registered."})],
                    tokens_in=150, tokens_out=25),
                ModelTurn(text="(mock) Raised plumbing complaint and sent the confirmation."),
            ],
            "electrical": [
                _call("check_can_raise"),
                ModelTurn(text=None, tool_calls=[ToolCall("raise_complaint", {"issue": "electrical"})], tokens_in=150, tokens_out=25),
                ModelTurn(text="(mock) Not raised: the room already has an open electrical issue."),
            ],
            "fan": [
                _call("check_can_raise"),
                ModelTurn(text="(mock) Not raised: the student's warning count of 3 is above the 2 limit."),
            ],
        }, slow),
    }
